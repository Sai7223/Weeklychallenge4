...
if download_and_extract_zip(zip_url):
    # Path to the dataset (adjusted for .csv.gz)
    dataset_path = 'data/Police_Bulk_Data_2014_20241027.csv.gz'

    # Check if the dataset exists and load only necessary columns
    if os.path.exists(dataset_path):
        df = pd.read_csv(dataset_path, usecols=['offensedate', 'offensedescription', 'offensegender', 'offenserace'], compression='gzip')
...
# Date range filter
if 'offensedate' in df.columns:  # Ensure column exists before accessing it
    with col1:
    ...
    st.error("Dataframe is empty or not loaded correctly.")
